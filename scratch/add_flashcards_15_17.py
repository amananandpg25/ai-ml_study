...
def inject_flashcards_for_week(w, days):
    ...
    for d in days:
        ...
        # Find the end of the day section
        ...
        if "Revision Flashcards" in day_body:
            print(f"  Day {d} already has flashcards, skipping.")
            continue
            
        # Find where to inject flashcards (after quiz section, or before takeaways)
        quiz_id = f"day-{d}-quiz-section"
        quiz_pos = day_body.find(quiz_id)
        if quiz_pos == -1:
            quiz_pos = day_body.find("quiz-section")
        if quiz_pos == -1:
            quiz_pos = day_body.find("quiz-block")
            
        if quiz_pos != -1:
            # Find the closing tag of this quiz container
            insert_pos = day_body.find('class="predict-block"', quiz_pos)
            if insert_pos == -1:
                insert_pos = day_body.find('id="tasks-section"', quiz_pos)
            if insert_pos == -1:
                insert_pos = day_body.find('class="task-block"', quiz_pos)
            if insert_pos == -1:
                insert_pos = day_body.find('sh2', quiz_pos)
                
            if insert_pos != -1:
                div_pos = day_body.rfind('<div', 0, insert_pos)
                if div_pos != -1:
                    insert_pos = div_pos
            else:
                insert_pos = day_body.rfind('</div>')
        else:
            # Fallback when no quiz section: insert before takeaways block
            takeaways_pos = day_body.find('class="takeaways"')
            if takeaways_pos != -1:
                insert_pos = takeaways_pos
            else:
                insert_pos = day_body.rfind('</div>')
                
        if insert_pos == -1:
            print(f"  WARNING: Could not find insertion point for Day {d}!")
            continue
            
        # Format the flashcards HTML
...
"""
        day_body = day_body[:insert_pos] + cards_html + "\n" + day_body[insert_pos:]
