...
if day13_start != -1 and day14_start != -1:
    ...
    res_grid_end = day13_block.find('<button class="complete-btn" id="btn-day-13"', res_grid_start)
    if res_grid_start != -1 and res_grid_end != -1:
        old_part = day13_block[res_grid_start:res_grid_end]
        
        new_part = """<div class="resources-grid">
  ...
  """
        ...
        new_day13_block = day13_block.replace(old_part, new_part)
        content = content.replace(day13_block, new_day13_block)
        print("Successfully updated Day 13 resources in week2.html!")
    else:
        print("Could not find resources-grid or complete-btn in Day 13 block.")
else:
